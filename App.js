...
//   component.
// - You will receive a warning message when the application loads, if the icon is
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <View>
          <TextInput style={{width: 150, height: 40, borderColor: 'black', borderWidth: 2}} onChangeText={(text) => this.textInputChanged(text)} />
          <Button onPress={() => this.getWeather()} title="Get the weather!" />
        </View>
        <View style={styles.currentWeather}>
          <Text style={styles.locationText}> {this.state.locationName} </Text>
          <Text style={styles.currentIcon}> {this.state.currentIcon && (<Icon size={40} name={icon(this.state.currentIcon)} />)} </Text>
